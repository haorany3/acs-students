...
public class RequestReader implements Request {
  ...
  private void authorize(User user, Door door) {
    ...
    if (!user.canSendRequest(now)) {
      authorized = false;
      addReason("user is not allowed to send request");
      return;
    }

    if (!user.canDoAction(action)) {
      authorized = false;
      addReason("User doesn't have permission to perform the action: " + action);
      return;
    }

    String fromSpace = door.getFromSpace();
    String toSpace = door.getToSpace();

    boolean canAccessFrom = user.canBeInSpace(fromSpace);
    boolean canAccessTo = user.canBeInSpace(toSpace);

    if (!canAccessFrom || !canAccessTo) {
      authorized = false;
      if (!canAccessFrom) {
        addReason("User can't access 'from' space: " + fromSpace);
      }
      if (!canAccessTo) {
        addReason("User can't access 'to' space: " + toSpace);
      }
      return;
    }
